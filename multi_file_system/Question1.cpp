...
public:
    ...
    bool updateFile(string fileName, int updatedFileSize) {
        ...
        if (numOfBlocks <= file->sizeInBlocks) {
            ...
            int n = (numOfBlocks - file->sizeInBlocks);
            int m = file->endBlock;
...
public:
    ...
    bool createFile(string fileName, int size) {
        ...
        // Marking the allocated blocks as free
        for (int i = startIndex; i < endIndex; i++) {
            this->disk[i].free = false;
    ...
    bool updateFile(string fileName, int updatedFileSize) {
        ...
        if (file->sizeInBlocks <= numOfBlocks) {
            // Move the file up above and free the extra blocks
            int n = (numOfBlocks - file->sizeInBlocks);
            int m = file->endBlock;
            ...
            if (startIndex == -1 && emptyBlockCnt >= numOfBlocks)
                startIndex = tmp;
    ...
    void printFinalStructure() {
        ...
        int i = 0;
        for (auto& itr: this->files) {
            string s = to_string(itr.second->startBlock) + " -> ";
            for (int j = 0; j < itr.second->sizeInBlocks; j++) {
                s += to_string(this->disk[j].nextBlock) + " -> "; 
            }
...
int main() {
    ...
    vector<string> operations = {"create", "create", "read", "read", "create", "delete", "read", "create", "update", "update", "create"};
    vector<string> fileNames = {"f1",      "f2",     "f2",   "f1",   "f3",     "f2",     "f2",   "f4",     "f3",     "f4",     "f5"};
    vector<int> params        = {300,    125,      0,       0,      100,       0,       0,      250,      230,      180,      190};

    int n = operations.size();
    // int n  = 9;

    // For contiguous memory allocation
    contiguousAlocation obj(64, 1024);
    for (int i = 0; i < n; i++) {
    ...
    indexedAllocation obj3(64, 1024);
    for (int i = 0; i < n; i++) {
        if (operations[i] == "create") {
            clock_t start = clock();
    ...
    modifiedContiguousAlocation obj4(64, 1024);
    for (int i = 0; i < n; i++) {
        if (operations[i] == "create") {
            clock_t start = clock();
    ...
    printf("    Read    | %13f | %13f | %13f | %20f\n", duration[0][1], duration[1][1], duration[2][1], duration[3][0]);
    printf("    Update  | %13f | %13f | %13f | %20f\n", duration[0][2], duration[1][2], duration[2][2], duration[3][0]);
    printf("    Delete  | %13f | %13f | %13f | %20f\n", duration[0][3], duration[1][3], duration[2][3], duration[3][0]);
